...
import { localizedSeedData, type SeedLocale } from "../lib/seed-data.ts";
...
export function seedDatabase(locale: SeedLocale = process.env.SEED_LOCALE === "en" ? "en" : "zh") {
  const { users: demoUsers, groups: seedGroups } = localizedSeedData(locale);
  ...
  try {
    ...
    const insertUser = database.prepare(`
      INSERT INTO "User" (id, displayName, avatarColor, email, passwordHash, phone, createdAt, updatedAt)
    ...
    for (const user of demoUsers) {
      insertUser.run(user.id, user.displayName, user.avatarColor, now, now);
    }
